...
very happy to offer a free tutorial database server access through `DataJoint.io <https://datajoint.io>`_.


Follow these simple instructions to signup and receive username and password to the tutorial database
...
1. Visit https://datajoint.io and click on Sign Up button. This will bring up a form.
2. Fill out your email and association.
3. Check "I want to sign up for a tutorial database access"
...
And that's it! Once you have the database access credentials, you can use that whenver you have to connect to database.

.. note::
  When you create a new schema, your schema must start with ``username_`` substituting in your username. For
  an example, if you signup with user name ``johndoe``, your schema name must start with ``johndoe_`` for you to have
  full control over tables defined in the schema. More details on this will be covered in the next section.

DataJoint.io_ also offers general database hosting service to host the entire data pipeline for one or more labs. If
you are interested in finding out more, be sure to visit us at https://datajoint.io!

Setting up local database server (Intermediate)
-----------------------------------------------

Alternatively you can install and set up a database server to run on your local machine. This offers a very
portable solution, but it may requires learning an additional tool to complete the task efficiently. If you
decide to go this route, we strongly recommend you to use `Docker <https://www.docker.com>`_ to download
and launch pre-configured database server we provide as a Docker image.
...
Setting up a database server for your lab (Advanced)
----------------------------------------------------

Finally, you might want to set up a database server on a dedicated hardward for a lab wide use. 
While having such a production level database server running on a dedicated hardware can certaintly offer great
flexibility, Properly setting up and maintaing such database server requires significant IT expertise and
...
setting up DataJoint library in Python or MATLAB to connect to the database! Pick your favorite language for
